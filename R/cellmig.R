...
get_fit <- function(x, control) {
    message("Step 1: Model selected \n")
    M <- stanmodels$M
    pars <- c("z_1", "z_2", "z_3", "z_4", "mu_well")
    
    sdata <- list(y=x$proc_x$v, 
                  N=x$proc_x$N[1], 
                  ...
                  prior_sigma_delta_M=control$prior_sigma_delta_M,
                  prior_sigma_delta_SD=control$prior_sigma_delta_SD)
    
    # fit model
    message("Step 2: Model fitting... \n")
...
get_summary <- function(x, f) {
    message("Step 3: Computing posterior summaries...\n")
    x <- x$proc_x
    ...
    p <- extract(f, par = "delta_t")$delta_t
    pmax <- apply(X = p, MARGIN = 2, FUN = get_pmax)
    delta_t$pmax <- pmax
    ...
    sigma_delta <- data.frame(summary(f, par="sigma_delta")$summary)
